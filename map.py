...
def save_goal_to_map(map, goal):
    map[goal[0], goal[1]] = 3
    map[goal[0] + 1, goal[1]] = 3
    map[goal[0], goal[1] + 1] = 3
    map[goal[0] + 1, goal[1] + 1] = 3
    map[goal[0] - 1, goal[1] + 1] = 3
    map[goal[0] + 1, goal[1] - 1] = 3
    map[goal[0], goal[1] - 1] = 3
    map[goal[0] - 1, goal[1]] = 3
...
def draw_map(world_map, goal):
    save_boundries_to_map(world_map)
    # world_map = [[world_map[j][i] for j in range(len(world_map))] for i in range(len(world_map[0]))] #transpose
    cmap = colors.ListedColormap(['white', 'Blue', 'red'])
    plt.figure()
    plt.pcolor(world_map, cmap=cmap, edgecolors='k', linewidth=0.05)
    # plt.xticks(np.arange(-X_LOWER_FIX , 700 - X_LOWER_FIX, 100))
    # plt.yticks(np.arange(-Y_LOWER_FIX, 380 - Y_LOWER_FIX, 100))
    save_goal_to_map(world_map, goal)
    plt.show()
